...
Потому, что даже зная его порядковый номер мы не знаем откуда и докуда читать (номера байтов).
...
Найдя значение в дереве и получи номер записи, мы можем считать его из файла. 
